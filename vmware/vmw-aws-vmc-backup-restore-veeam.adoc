...
image:aws-vmc-veeam-037.png[Solution Architecture Diagram]
...
+
image:aws-vmc-veeam-004.png[Open the Add Veeam backup proxy wizard]

. In the *Add VMware Proxy* wizard click the *Add New...* button to add a new proxy server.
+
image:aws-vmc-veeam-005.png[Select to add a new server]
...
+
image:aws-vmc-veeam-006.png[Fills prompts to add new server]

. Back in the *New VMware Proxy* wizard, choose a Transport Mode. In our case we chose *Automatic Selection*. 
+
image:aws-vmc-veeam-007.png[Select transport mode]

. Select the Connected datastores that you want the VMware Proxy to have direct access to. 
+
image:aws-vmc-veeam-008.png[Select a server for VMware Proxy]
+
image:aws-vmc-veeam-009.png[Select datastores to access]
...
+
image:aws-vmc-veeam-010.png[Configure network traffic rules]
=====
...
+
image:aws-vmc-veeam-001.png[Create FSx ONTAP File System]

. Select *Amazon FSx ONTAP* and then *Next* to continue. 
+
image:aws-vmc-veeam-002.png[Select Amazon FSx ONTAP]
...
+
image:aws-vmc-veeam-003.png[Fill out File System Info]
...
+
image:aws-vmc-veeam-011.png[iSCSI Initiator Discovery]
...
+
image:aws-vmc-veeam-012.png[Connect iSCSI Initiator to LUN]
...
+
image:aws-vmc-veeam-013.png[Windows Disk Management]
...
+
image:aws-vmc-veeam-014.png[Create a new Backup Repository]
...
+
image:aws-vmc-veeam-015.png[Select Backup Repository server]
...
+
image:aws-vmc-veeam-016.png[Choose Mount server]
...
+
image:aws-vmc-veeam-017.png[Application processing settings]
...
+
image:aws-vmc-veeam-018.png[Long-term retention policy]
=====
...
+
image:aws-vmc-veeam-019.png[Restore SQL Server databases]

. In the Microsoft SQL Server Database Restore Wizard select a restore point from the list and click on *Next*.
+
image:aws-vmc-veeam-020.png[Select a restore point from the list]
...
+
image:aws-vmc-veeam-021.png[Click on Browse to launch Veeam Explorer]
...
+
image:aws-vmc-veeam-022.png[Select instant recovery restore point]
...
+
image:aws-vmc-veeam-023.png[Select switchover type]

. The recovery process can be monitored from Veeam Explorer.
+
image:aws-vmc-veeam-024.png[monitor sql server recovery process]
=====
...
+
image:aws-vmc-veeam-025.png[Restore Oracle databases]

. In the Oracle Database Restore Wizard select a restore point from the list and click on *Next*.
+
image:aws-vmc-veeam-026.png[Select a restore point from the list]
...
+
image:aws-vmc-veeam-027.png[Click on Browse to launch Veeam Explorer]
...
. In the Veeam Backup and Replication console, navigate to the list of Oracle backups, right click on a server and select *Restore application items* and then *Oracle databases...*.
+
image:aws-vmc-veeam-032.png[Restore Oracle databases]

. In the Oracle Database Restore Wizard select a restore point from the list and click on *Next*.
+
image:aws-vmc-veeam-033.png[Select a restore point from the list]
...
+
image:aws-vmc-veeam-034.png[Select a restore point from the list]
...
+
image:aws-vmc-veeam-035.png[Select a restore point from the list]
...
....
+
image:aws-vmc-veeam-036.png[Select a restore point from the list]
...
// NetApp Solutions restructuring (jul 2025) - renamed from ehc/aws-vmc-veeam-fsx-solution.adoc
