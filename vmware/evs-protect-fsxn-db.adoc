...
[.lead]
The following section shows how to protect databases running on VMware VMDKs with SnapCenter for SQL (SCSQL). 
...
+
image:evs-protect-fsxn-033.png[Figure representing the dialog for the above step]
...
image:evs-protect-fsxn-034.png[Figure representing the dialog for the above step]

image:evs-protect-fsxn-035.png[Figure representing the dialog for the above step]
...
+
image:evs-protect-fsxn-036.png[Figure representing the dialog for the above step]

. In the Restore page, select the option "Restore the database to the same host where the backup was created". 
+
image:evs-protect-fsxn-037.png[Figure representing the dialog for the above step]

. Select the option to use a log backup to a point in time. 
+
image:evs-protect-fsxn-038.png[Figure representing the dialog for the above step]

. Select "Overwrite database with same name during restore, Create transaction log backup before restore", and select "Next".
+
image:evs-protect-fsxn-039.png[Figure representing the dialog for the above step]

. Select the post-restore state you wish the database to assume from the list options and select "Next".
+
image:evs-protect-fsxn-040.png[Figure representing the dialog for the above step]

. On the summary page, select "Finish".
+
image:evs-protect-fsxn-041.png[Figure representing the dialog for the above step]

. Monitor the job to completion.
+
image:evs-protect-fsxn-042.png[Figure representing the dialog for the above step]

. Verify that the deleted tables have been successfully restored.
+
image:evs-protect-fsxn-043.png[Figure representing the dialog for the above step]
