...
For information about the administrator team that performs these service upgrades, see the section link:gcp-gcnv-service-operation.html["Service operation."]
...
For more information about architecture, see the section link:gcp-gcnv-arch-detail.html["Google Cloud NetApp Volumes architecture."]
...
Most insecure in-flight protocols such as telnet, NDMP, and so on are disabled by default. DNS, however, is not encrypted by Google Cloud NetApp Volumes (no DNS Sec support) and should be encrypted by using external network encryption when possible. See the section link:gcp-gcnv-data-encrypt-in-transit.html["Data encryption in transit"] for more information about securing data in-flight.

For information about NAS protocol encryption, see the section link:gcp-gcnv-data-encrypt-in-transit.html#nas-protocols["NAS protocols."]
...
// NetApp Solutions restructuring (jul 2025) - renamed from ehc/ncvs-gc-security-considerations-and-attack-surfaces.adoc
